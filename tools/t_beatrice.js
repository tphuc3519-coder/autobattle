...
(async () => {
  ...
  ok(C.minyaDmg === 15 && near(C.minyaCd, 1, .001),
     `Minya ${C.minyaDmg} dmg, mot don moi ${C.minyaCd}s nguoi choi`);
  ok(near(C.stunOdds, .20, .001) && near(C.stun, .5, .001),
     `Minya Stun ${Math.round(C.stunOdds * 100)}% x ${C.stun}s`);
  ok(C.shamacDmg === 10 && near(C.shamacCd, 5, .001), `Al Shamac ${C.shamacDmg} dmg / ${C.shamacCd}s`);
  ok(near(C.murakCd, 7, .001) && near(C.murakT, 3, .001), `Murak ${C.murakCd}s cd, giu ${C.murakT}s`);
  ok(near(C.emtCd, 7, .001) && near(C.emtT, 2, .001), `E.M.T ${C.emtCd}s cd, giu ${C.emtT}s`);
  ok(C.ultN === 3 && C.ultDmg === 50 && near(C.ultCd, 8, .001),
     `El Minya ${C.ultN} tia x ${C.ultDmg} dmg / ${C.ultCd}s`);
  ok(near(C.eroDps, 2, .001) && C.eroMax === 3 && near(C.eroT, 3, .001),
  ...
  ok(minya.dmg === 15, `mot mui Minya an dung ${minya.dmg} dmg`);
  ...
  ok(near(murak.take, C.murakRes, .001) && murak.hit === 80,
     `don 100 dmg + choang 2s => an dung ${murak.hit} dmg va khong choang`);
  ok(murak.stacked === 64,
     `hai lop giam sat thuong NHAN chong: -20% Shamac Weakness + -20% Murak => ${murak.stacked} dmg, khong phai 60 (cong phan tram)`);
  ...
  ok(emt.refl100 === 20 && emt.refl50 === 10 && emt.refl200 === 40,
     `phan lai 20%: chan 50 => ${emt.refl50}, chan 100 => ${emt.refl100}, chan 200 => ${emt.refl200}`);
  ok(emt.dotTaken === 0 && emt.dotRefl === 1,
  ...
  ok(ult.hit1 === 50, `mot mui an dung ${ult.hit1} dmg`);
  ok(ult.direct === 200 && ult.ero3 === 3 && ult.ero4 === 3,
     `Mana Erosion cong don toi ${ult.ero3} stack roi dung han (mui thu tu van ${ult.ero4})`);
  ok(near(ult.dps, 6, .001), `ba stack cung chay: ${ult.dps} HP moi giay nguoi choi`);
