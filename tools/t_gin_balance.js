/* Cân bằng Captain Ginyu — đo TỈ LỆ THẮNG thật, không đoán.
   Chạy: node tools/t_gin_balance.js [số lượt mỗi cặp]

   Bản sao của `t_bea_balance.js`, chỉ đổi người được đo. Giữ nguyên cách đo để hai bảng
   so được với nhau: cùng trần thời gian, cùng số trang mở một lúc, cùng cột "máu còn lại".
...
const DICH = ['kono', 'chichi', 'tsubasa', 'shika', 'suzune', 'sakura', 'dora', 'superman',
              'beatrice', 'tanjiro', 'gojo', 'conan', 'isagi'];
...
(async () => {
  ...
  const danh = async (dich, seed) => {
    ...
    const r = await page.evaluate(moc => new Promise(res => {
      ...
      const id = setInterval(() => {
        if (G.over || G.t > moc) {
          clearInterval(id);
          /* THẮNG THUA TÍNH THEO HỒN, KHÔNG THEO THÂN XÁC. Sau cú CHANGE thì object mang
             key:'ginyu' có thể đang do hồn đối thủ điều khiển, còn hồn Ginyu thì ngồi
             trong thân xác bên kia — đọc `f.key` là đo NGƯỢC hẳn kết quả ở đúng mấy trận
             anh dùng chiêu tủ. Cùng luật với `compSoul()` của giải đấu. */
          const hon = f => f.gnSoul || f.key;
          const song = G.fighters.filter(f => f && !f.summon);
          const b = song.find(f => hon(f) === 'ginyu');
          const d = song.find(f => hon(f) !== 'ginyu') || song[0];
          res({ t: +G.t.toFixed(0), over: !!G.over,
                win: G.winner ? hon(G.winner) : null,
                swap: !!(b && b.swapAs),
                bHp: b ? Math.round(b.hp) : 0, dHp: d ? Math.round(d.hp) : 0,
                bMax: b ? b.maxHp : 800 });
        }
      }, 50);
      setTimeout(() => { clearInterval(id); res({ t: -1, over: false, win: null, swap: false, bHp: 0, dHp: 0, bMax: 800 }); }, 120000);
    }), MOC);
    await page.close();
  ...
  let thang = 0, tong = 0, hoa = 0, mauCon = 0, doiXac = 0;
  const theoDich = {};
  for (const r of out) {
    const k = r.dich;
    ...
    if (r.win === 'ginyu') { thang++; theoDich[k].w++; mauCon += r.bHp / r.bMax; theoDich[k].hp.push(Math.round(r.bHp / r.bMax * 100)); }
    else theoDich[k].l++;
  }
  console.log('doi thu      | Ginyu thang    | thua | het gio | mau con lai khi thang');
  for (const k of DICH) {
    const t = theoDich[k];
  ...
  console.log(`So tran ket thuc trong THAN XAC DI MUON (CHANGE trung): ${doiXac}/${tong}`);
