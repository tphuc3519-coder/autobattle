/* Cân bằng Haruno Sakura — đo TỈ LỆ THẮNG thật, không đoán.
   Chạy: node tools/t_sak_balance.js [số lượt mỗi cặp]
...
(async () => {
  ...
  const danh = async (dich, seed) => {
    ...
    await page.click('#listA .cTile[data-key="sakura"]');
    ...
    const r = await page.evaluate(moc => new Promise(res => {
      ...
      const id = setInterval(() => {
        if (G.over || G.t > moc) {
          ...
          const b = song.find(f => f.key === 'sakura');
          const d = song.find(f => f.key !== 'sakura') || song[0];
  ...
  for (const r of out) {
    ...
    if (r.win === 'sakura') { thang++; theoDich[k].w++; mauCon += r.bHp / r.bMax; theoDich[k].hp.push(Math.round(r.bHp / r.bMax * 100)); }
    else theoDich[k].l++;
  }
  console.log('doi thu      | Sakura thang   | thua | het gio | mau con lai khi thang');
  for (const k of DICH) {
    const t = theoDich[k];
